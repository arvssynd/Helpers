...
namespace Helpers
{
    public static class OpenXmlExcelGenerator
    {
        public static Stream GenerateExcel<T>(ICollection<T> data)
        {
            Stream fs = new MemoryStream();
            using (SpreadsheetDocument package = SpreadsheetDocument.Create(fs, SpreadsheetDocumentType.Workbook))
            {
        ...
        private static void CreatePartsForExcel<T>(SpreadsheetDocument document, ICollection<T> data)
        {
        ...
        private static SheetData GenerateSheetdataForDetails<T>(ICollection<T> data)
        {
            SheetData sheetData1 = new SheetData();
            var count = 0;
            foreach (T testmodel in data)
            {
                if (count == 0)
                {
                    sheetData1.Append(CreateHeaderRowForExcel(testmodel));
                }
        ...
        private static void GenerateWorkbookStylesPartContent(WorkbookStylesPart workbookStylesPart1)
        {
            ...
            CellFormat cellFormat6 = new CellFormat() { NumberFormatId = (UInt32Value)22, FontId = (UInt32Value)0U, FillId = (UInt32Value)0U, BorderId = (UInt32Value)1U, FormatId = (UInt32Value)0U, ApplyBorder = true };
        ...
        private static Row CreateHeaderRowForExcel(object data)
        {
            Row workRow = new Row();

            foreach (var property in data.GetType().GetProperties())
            {
                ExportExcelHeader headerAttribute = (ExportExcelHeader)Attribute.GetCustomAttribute(property, typeof(ExportExcelHeader));
                workRow.Append(CreateCell(headerAttribute != null ? headerAttribute.Name : property.Name.ToString(), 2U));
        ...
        private static Row GenerateRowForChildPartDetail(object testmodel)
        {
            ...
            {
                var typeType = property.PropertyType.Name;
                if (typeType == "DateTime")
                {
                    var date = (DateTime)property.GetValue(testmodel, null);
                    var offset = TimeZoneInfo.Local.GetUtcOffset(date);
                    var dd = date.AddMinutes(offset.TotalMinutes);
                    Cell cell = new Cell();
                    cell.CellValue = new CellValue(dd);
                    cell.StyleIndex = 4;
                ...
                {
                    tRow.Append(CreateCell(StripHtml(property.GetValue(testmodel, null)?.ToString()), typeType));
                }
        ...
        private static Cell CreateCell(string text, string typeType)
        {
            Cell cell = new Cell();
            cell.StyleIndex = 1U;
            cell.DataType = ResolveCellDataTypeOnValue(text, typeType);
        ...
        private static EnumValue<CellValues> ResolveCellDataTypeOnValue(string text, string typeType)
        {
            switch (typeType)
            {
                case "Int":
    ...
    public class ExportExcelHeader : Attribute
    {
